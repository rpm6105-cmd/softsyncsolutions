...
window.renderLive = () => {
    ...
    const sig = `
        <div style="text-align:right;">
            <p style="font-family:'Great Vibes',cursive;font-size:2.2rem;color:${C.dark};margin:0 0 2px;line-height:1.1;">Rohith P.M.</p>
            <div style="width:120px;height:1px;background:${C.gold};margin:0 0 5px auto;"></div>
            <p style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:0.14em;color:${C.light};">Director, Softsync Solutions</p>
        </div>`;

    /* ── FOOTER BAR ── */
    const footer = `
        <div style="background:${C.header};padding:8px 18mm;display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:0.5rem;color:rgba(255,255,255,0.35);letter-spacing:0.12em;text-transform:uppercase;">www.softsyncsolutions.in</span>
            <div style="width:20px;height:1px;background:${C.gold};opacity:0.5;"></div>
            <span style="font-size:0.5rem;color:rgba(255,255,255,0.35);letter-spacing:0.12em;text-transform:uppercase;">Trusted Partner in Digital Transformation</span>
        </div>`;
    ...
    if (mode === 'quotation' || mode === 'invoice') {
        ...
        const statusBg    = isInv ? '#FEF3C7' : '#F5EFE3';
        const statusColor = isInv ? '#92400E' : '#78613A';

        let subtotal = 0;
        const rows = activeItems.map((item, idx) => {
            const lt = item.qty * item.rate; subtotal += lt;
            return `<tr>
                <td style="padding:10px 14px;font-size:0.7rem;color:${C.light};vertical-align:top;">${idx + 1}</td>
                <td style="padding:10px 8px;font-size:0.78rem;color:${C.dark};font-weight:500;line-height:1.4;">${item.desc}</td>
                <td style="padding:10px 8px;font-size:0.78rem;color:${C.mid};text-align:center;">${item.qty}</td>
                <td style="padding:10px 8px;font-size:0.78rem;color:${C.mid};text-align:right;">₹${item.rate.toLocaleString('en-IN')}</td>
                <td style="padding:10px 14px;font-size:0.78rem;font-weight:700;color:${C.dark};text-align:right;">₹${lt.toLocaleString('en-IN')}</td>
            </tr>`;
        }).join('');

        document.getElementById('document-preview').innerHTML = `
        <div style="background:${C.bg};min-height:297mm;position:relative;font-family:'Inter',sans-serif;">

            <!-- HEADER -->
            <div style="background:${C.header};padding:10mm 18mm 9mm;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    <div>
                        <img src="${LOGO}" style="height:40px;object-fit:contain;display:block;margin-bottom:9px;">
                        <div style="font-size:0.58rem;color:rgba(255,255,255,0.38);line-height:1.8;letter-spacing:0.01em;">${company.address}<br>${company.email} &nbsp;·&nbsp; ${company.phone}</div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:0.5rem;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:${C.gold};margin-bottom:6px;">${label}</div>
                        <div style="font-size:3rem;font-weight:800;color:#fff;letter-spacing:-0.05em;line-height:1;font-family:'Outfit',sans-serif;">A00001</div>
                    </div>
                </div>
                <!-- Gold rule -->
                <div style="margin-top:8mm;height:1px;background:linear-gradient(90deg,${C.gold},rgba(184,151,90,0.1));"></div>
            </div>

            <!-- FROM / BILL TO / DETAILS -->
            <div style="display:grid;grid-template-columns:1fr 1fr 0.8fr;background:${C.cream};border-bottom:1px solid ${C.border};">
                <div style="padding:5mm 18mm 5mm;">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:6px;">From</div>
                    <div style="font-size:0.88rem;font-weight:800;color:${C.dark};margin-bottom:3px;letter-spacing:-0.01em;">${company.name}</div>
                    <div style="font-size:0.65rem;color:${C.mid};line-height:1.6;">${company.address}</div>
                </div>
                <div style="padding:5mm 8mm;border-left:1px solid ${C.border};">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:6px;">Bill To</div>
                    <div style="font-size:0.88rem;font-weight:800;color:${C.dark};margin-bottom:3px;letter-spacing:-0.01em;">${client}</div>
                    <div style="font-size:0.65rem;color:${C.mid};line-height:1.6;">${addr}</div>
                    ${phone ? `<div style="font-size:0.65rem;color:${C.mid};margin-top:2px;">${phone}</div>` : ''}
                </div>
                <div style="padding:5mm 8mm;border-left:1px solid ${C.border};background:${C.goldLight};">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:7px;">Details</div>
                    <div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="font-size:0.63rem;color:${C.mid};">Issue Date</span><span style="font-size:0.63rem;font-weight:700;color:${C.dark};">${dateStr}</span></div>
                    <div style="display:flex;justify-content:space-between;margin-bottom:7px;"><span style="font-size:0.63rem;color:${C.mid};">Valid Till</span><span style="font-size:0.63rem;font-weight:700;color:${C.dark};">${validStr}</span></div>
                    <div style="padding-top:6px;border-top:1px solid ${C.borderDark};display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:0.58rem;color:${C.mid};font-weight:600;">Status</span>
            ...
            ${subject ? `<div style="padding:3.5mm 18mm;background:${C.bg};border-bottom:1px solid ${C.border};">
                <span style="font-size:0.58rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:${C.light};">Re: </span>
                <span style="font-size:0.75rem;color:${C.dark};font-weight:600;">${subject}</span>
            </div>` : ''}

            <!-- LINE ITEMS TABLE -->
            <div style="padding:0 18mm;background:${C.bg};">
                <table style="width:100%;border-collapse:collapse;margin-top:5mm;">
                    <thead>
                        <tr style="background:${C.header};">
                            <th style="padding:8px 14px;font-size:0.58rem;font-weight:700;letter-spacing:0.12em;color:${C.gold};text-align:left;width:24px;">#</th>
                            <th style="padding:8px 8px;font-size:0.58rem;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,0.55);text-align:left;">Description</th>
                            <th style="padding:8px 8px;font-size:0.58rem;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,0.55);text-align:center;width:44px;">Qty</th>
                            <th style="padding:8px 8px;font-size:0.58rem;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,0.55);text-align:right;width:80px;">Rate</th>
                            <th style="padding:8px 14px;font-size:0.58rem;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,0.55);text-align:right;width:88px;">Amount</th>
                        </tr>
                    </thead>
                    <tbody style="border-left:1px solid ${C.border};border-right:1px solid ${C.border};">
                        ${rows || `<tr><td colspan="5" style="padding:22px;text-align:center;color:${C.light};font-size:0.72rem;font-style:italic;border:1px solid ${C.border};">No items added yet</td></tr>`}
                    </tbody>
                </table>
                ...
                <div style="display:flex;justify-content:flex-end;margin-bottom:6mm;">
                    <div style="min-width:260px;border:1px solid ${C.border};border-top:none;">
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:${C.cream};">
                            <span style="font-size:0.65rem;color:${C.mid};font-weight:600;letter-spacing:0.04em;">SUBTOTAL</span>
                            <span style="font-size:0.78rem;color:${C.dark};font-weight:600;">₹${subtotal.toLocaleString('en-IN')}</span>
                        </div>
                        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:${C.header};">
                            <span style="font-size:0.65rem;color:rgba(255,255,255,0.55);font-weight:600;letter-spacing:0.06em;">TOTAL (INR)</span>
            ...
            <!-- TERMS & SIGNATURE -->
            <div style="padding:0 18mm 6mm;background:${C.bg};display:grid;grid-template-columns:1.2fr 0.8fr;gap:8mm;align-items:end;">
                <div style="background:${C.cream};border:1px solid ${C.border};border-radius:4px;padding:4mm;">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.gold};margin-bottom:6px;">Terms & Conditions</div>
                    ${['Quoted prices are final and all-inclusive.','Validity of this quotation is 21 days.','Project kickoff only after advance payment.'].map(t =>
                        `<div style="display:flex;gap:6px;margin-bottom:4px;">
                            <span style="color:${C.gold};font-size:0.65rem;margin-top:1px;">·</span>
                            <span style="font-size:0.63rem;color:${C.mid};line-height:1.5;">${t}</span>
                        </div>`).join('')}
                </div>
                ${sig}
            </div>
        ...
        const section = (num, title, content) => `
            <div style="margin-bottom:5mm;padding-bottom:5mm;border-bottom:1px solid ${C.border};">
                <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:3mm;">
                    <div style="width:20px;height:20px;background:${C.header};border-radius:3px;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">
                        <span style="font-size:0.58rem;font-weight:800;color:${C.gold};">${num}</span>
                    </div>
                    <div style="font-size:0.62rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:${C.dark};padding-top:3px;">${title}</div>
                </div>
                <div style="padding-left:30px;font-size:0.74rem;color:${C.mid};line-height:1.7;">${content || `<span style="color:${C.light};font-style:italic;">Not specified</span>`}</div>
            </div>`;
        ...
        <div style="background:${C.bg};min-height:297mm;position:relative;font-family:'Inter',sans-serif;">
            ...
            <div style="background:${C.header};padding:10mm 18mm 9mm;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    ...
                    <div style="text-align:right;">
                        <div style="font-size:0.5rem;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:${C.gold};margin-bottom:4px;">Prepared By</div>
                        <div style="font-size:0.82rem;font-weight:700;color:#fff;">${company.name}</div>
                        <div style="font-size:0.58rem;color:rgba(255,255,255,0.38);margin-top:2px;">${company.email} &nbsp;·&nbsp; ${company.phone}</div>
                    </div>
                </div>
                <div style="margin-top:8mm;padding-top:7mm;border-top:1px solid rgba(255,255,255,0.08);">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.2em;color:${C.gold};margin-bottom:5px;">Project Proposal</div>
            ...
            <div style="height:2px;background:linear-gradient(90deg,${C.gold} 0%,${C.goldMid} 50%,rgba(184,151,90,0.05) 100%);"></div>

            <!-- META ROW -->
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;background:${C.cream};border-bottom:1px solid ${C.border};">
                <div style="padding:4.5mm 18mm;">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:4px;">Prepared For</div>
                    <div style="font-size:0.85rem;font-weight:800;color:${C.dark};letter-spacing:-0.01em;">${client}</div>
                    ${addr ? `<div style="font-size:0.63rem;color:${C.mid};margin-top:2px;line-height:1.5;">${addr}</div>` : ''}
                </div>
                <div style="padding:4.5mm 8mm;border-left:1px solid ${C.border};">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:4px;">Date</div>
                    <div style="font-size:0.85rem;font-weight:700;color:${C.dark};">${dateStr}</div>
                    ${timeline ? `<div style="font-size:0.62rem;color:${C.mid};margin-top:2px;">Timeline: ${timeline}</div>` : ''}
                </div>
                <div style="padding:4.5mm 8mm;border-left:1px solid ${C.border};background:${C.goldLight};">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:4px;">Project Cost</div>
                    <div style="font-size:1.1rem;font-weight:800;color:${C.header};font-family:'Outfit',sans-serif;letter-spacing:-0.02em;">₹${parseFloat(cost || 0).toLocaleString('en-IN')}</div>
                </div>
            </div>

            <!-- SECTIONS -->
            <div style="padding:6mm 18mm 0;background:${C.bg};">
                ${section('1', 'Scope of Work', scope.replace(/\n/g, '<br>'))}
                ${section('2', 'Deliverables', deliverables.replace(/\n/g, '<br>'))}
                ${section('3', 'Payment Terms', payment.replace(/\n/g, '<br>'))}
                ${notes ? `<div style="background:${C.goldLight};border:1px solid ${C.borderDark};border-radius:4px;padding:4mm;margin-bottom:5mm;">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.gold};margin-bottom:4px;">Additional Notes</div>
                    <div style="font-size:0.7rem;color:${C.mid};line-height:1.6;">${notes}</div>
                </div>` : ''}
            </div>

            <!-- FOOTER AREA -->
            <div style="padding:0 18mm 6mm;background:${C.bg};display:flex;justify-content:space-between;align-items:flex-end;">
                <div style="font-size:0.63rem;color:${C.light};line-height:1.8;max-width:200px;">
    ...
    ══════════════════════════════════════════════ */
        ...
        <div style="background:${C.bg};min-height:297mm;position:relative;font-family:'Inter',sans-serif;">
            ...
            <!-- HEADER -->
            <div style="background:${C.header};padding:7mm 18mm;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <img src="${LOGO}" style="height:38px;object-fit:contain;">
                    <div style="text-align:right;">
                        <div style="font-size:0.62rem;color:rgba(255,255,255,0.38);line-height:1.8;letter-spacing:0.01em;">${company.address}<br>${company.email} &nbsp;·&nbsp; ${company.phone}</div>
                    </div>
                </div>
            </div>
            <div style="height:2px;background:linear-gradient(90deg,${C.gold},rgba(184,151,90,0.08));"></div>

            <!-- TO / DATE ROW -->
            <div style="background:${C.cream};padding:4mm 18mm;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid ${C.border};">
                <div>
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:4px;">To</div>
                    <div style="font-size:0.92rem;font-weight:800;color:${C.dark};letter-spacing:-0.01em;">${client}</div>
                    ${addr ? `<div style="font-size:0.65rem;color:${C.mid};margin-top:2px;">${addr}</div>` : ''}
                </div>
                <div style="text-align:right;">
                    <div style="font-size:0.52rem;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;color:${C.light};margin-bottom:4px;">Date</div>
                    <div style="font-size:0.78rem;font-weight:700;color:${C.dark};">${dateStr}</div>
                </div>
            </div>

            <!-- LETTER BODY -->
            <div style="padding:8mm 18mm;background:${C.bg};min-height:165mm;">
                ${subject ? `<div style="font-size:0.78rem;font-weight:700;color:${C.dark};margin-bottom:7mm;padding-bottom:4mm;border-bottom:1px solid ${C.border};">
                    <span style="color:${C.light};font-weight:600;">Re: </span>${subject}
                </div>` : ''}
                <div style="font-size:0.78rem;color:${C.light};font-style:italic;line-height:2;">[ Letter body goes here ]</div>
            </div>

            <!-- SIGNATURE -->
            <div style="padding:0 18mm 6mm;background:${C.bg};display:flex;justify-content:flex-end;">
                ${sig}
            </div>
